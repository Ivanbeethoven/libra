...
| `%(worktreepath)` | checkout 了该 ref 的 worktree 绝对路径；否则为空。跨所有 worktree 从各自的 scoped HEAD row 解析，因此在 linked worktree 中 checkout 的分支即使命令在别处运行也会返回该 worktree 的路径；分支未在任何 worktree checkout 时为空。 |
