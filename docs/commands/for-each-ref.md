...
| `%(worktreepath)` | Absolute path of the worktree that has this ref checked out; empty otherwise. It is resolved across all worktrees from each worktree's own scoped HEAD row, so a branch checked out in a linked worktree reports that worktree's path even when the command runs elsewhere; empty when the branch is checked out nowhere. |
