...
| 2026-07-06 | Task A9（AG-24 closeout） | 进行中（session 362f0f7a 已认领） | — | — | in-progress 认领标记：避免并行 session 重复实施 A9。范围：agent.md 规划/已注册 target 一致性、docs/commands/agent.md+zh-CN/COMPATIBILITY.md/error-codes.md/INDEX.md 同步、A8.5 retention/raw-export/audit 用户文档、out-of-scope banner（memory/sandbox/web-api）、fixtures 溯源版本核对、runtime source-of-truth 边界、旧 claudecode 不复活、release notes 分级（enabled/preview/deferred）。验证：compat_agent_docs_contract/run_non_exhaustive_guard/matrix_alignment/error_codes_doc_sync + 否定断言 + fmt/clippy/test-all。完成后本行改写为验收记录 |
