...
| 邮件 plumbing | P2-02 `mailinfo <msg> <patch> < mail` 的 bounded decode/body+patch split/output safety；P2-03 `format-patch` 的 `-1`/root/config/thread/MIME/diff/upstream suppression 与 Libra↔Git `am` 双向互操作 | 1 | 已实现（独立 Cargo 黑盒 targets） | Cargo: `compat_mailinfo_basic`, `compat_mail_am_basic`, `compat_format_patch_mail_roundtrip` |
