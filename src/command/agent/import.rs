...
use crate::{
    internal::{
        ai::{
            agent_import::{
                ExistingSessionOwnershipSnapshot, ImportError, ImportPreparationContext,
                ImportProgressError, ImportRequest, ImportSummary, import_prepared,
...
impl BatchResult {
    fn complete(summary: ImportSummary) -> Self {
        let status = if summary.checkpoints_written == 0 {
            "noop"
        } else {
            "imported"
        };
    ...
    fn partial(summary: ImportSummary) -> Self {
        Self {
            status: "partial",
            summary,
        }
...
fn validate_session_id(value: &str) -> CliResult<()> {
    ...
    if valid {
        ...
        Err(CliError::command_usage(
            "invalid provider session id (expected alphanumeric/dash/underscore, at most 128 characters)",
        )
        .with_stable_code(StableErrorCode::CliInvalidArguments))
    }
}
...
fn claude_session_id_from_filename(path: &Path) -> Option<String> {
    ...
    validate_session_id(&stem).ok().map(|()| stem)
}
...
pub async fn execute_safe(args: ImportArgs, output: &OutputConfig) -> CliResult<()> {
    ...
    for (candidate_index, candidate) in candidates.iter().enumerate() {
        // Each completed candidate can enqueue asynchronous object-index
        // writes against this repository database. Drain those writes before
        // the next candidate starts its lease transaction; otherwise a fast
        // batch can race its own indexer and surface a spurious SQLite-busy
        // failure. The shared import deadline keeps this coordination bounded.
        if candidate_index > 0 && !ClientStorage::wait_for_background_tasks_until(deadline).await {
        ...
        let result = async {
            ...
            import_prepared(&conn, &storage_root, request, deadline).await
        }
        .await;
        match result {
            Ok(summary) if summary.partial => {
                partial_results.push(BatchResult::partial(summary));
                failures.push(safe_failure(candidate, &ImportError::LeaseBusy.into()));
            }
            Ok(summary) => results.push(BatchResult::complete(summary)),
            Err(error) => {
                if let Some(progress) = error.downcast_ref::<ImportProgressError>() {
                    partial_results.push(BatchResult::partial(progress.summary.clone()));
                }
                if discovered_batch && is_discovery_skip(&error) {
                    skipped.push(safe_skip(candidate, &error));
                } else {
                    failures.push(safe_failure(candidate, &error));
                }
    ...
    if !payload.failures.is_empty() {
        ...
        let message = if candidates.len() == 1
            ...
            && payload.partial_results.is_empty()
        {
            "agent import failed; resolve the reported stable error code and retry".to_string()
        } else {
            format!(
                "agent import completed partially: {} succeeded, {} made partial progress, {} failed; rerun failed selections after resolving the reported stable error codes",
                payload.results.len(),
    ...
    if !output.quiet {
        println!(
            "Imported {} session(s), skipped {} session(s), {} turn checkpoint(s); next cursor: {}",
            payload.results.len(),
            payload.skipped.len(),
            payload
                .results
