...
| `install_alias_test.rs` + `install_alias_smoke.sh` + `install_alias_windows.ps1` | IX-01 / plan-20260714 PD-10 | Unix symlink 与 Windows `lba.cmd` 安装器：same-version 缺失修复/幂等、CLI/env opt-out、用户文件保护、卸载清理；Windows 通过 PowerShell 分支执行，非 Windows gate-skip |
