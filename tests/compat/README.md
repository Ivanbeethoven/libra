...
| `config_defaults_semantics_test.rs` | P1-05a/P1-05c (plan-20260708) | 高影响 Git config 默认值：`init.defaultBranch`、`pull.rebase` / `branch.<name>.rebase`、`pull.ff=true|false|only` 与 `fetch.prune` / `remote.<name>.prune`（远程键跨 scope 优先、Git 数值布尔、CLI 覆盖、`--all` 联网前整体校验）覆盖 local/global/system、变量名大小写、空/无效值、真实 rebase/prune 行为及 fetch 前 fail-closed |
