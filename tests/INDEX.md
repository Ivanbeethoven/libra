...
| `db_migration_test` | 1 | SQLite schema bootstrap + migration round-trip, including M4 import identity/tombstone empty up→down→up, non-empty identity and tombstone rollback refusal, and uniqueness/state failure matrix | `src/internal/db.rs`, `sql/` |
